...
Dynamic String Array: dynamicString.cpp Ð 20 pts
*******
*****
****
***
...
int main ()
{
  ...
  pNewString = new string[size];

  yoliesProgramHeader();
  displayExplanation();

  if(elementsInArray == size)
    {pNewString = expand(pNewString,size);}
  ...
  do
  {
    ...
    if(choice > 0 && choice < 4)
    {
      ...
      if(choice == 3)
      {
        cout << "\n\tPlease enter a subscript to insert " << petName << " at: ";
        cin >> index;
      }
...
void sort(string *mainFunctArray, int elementsInArray)
{
  string *minVal;
  int minIndex;
  ...
  {
    *(minVal) = *(mainFunctArray + i);
    minIndex = i;

    for(int i2 = i + 1; i2 < elementsInArray; i2++)
    {
      if((mainFunctArray[i2]) < *(minVal))
      {
        *(minVal) = mainFunctArray[i2];
        minIndex = i2;
      }
    }

    mainFunctArray[minIndex] = mainFunctArray[i];
    mainFunctArray[i] = *(minVal);
  }

}

void printList(string *mainFunctArray, int elementsInArray)
{
  for(int i = 0; i < elementsInArray; i++)
  {
    cout << "\n\n\t" << i + 1 << ".\t" << *(mainFunctArray + i) << endl;
  }
}


bool removeFrom(string *mainFunctArray, int index, int elementsInArray)
{
  int maxSize = elementsInArray;

  for(int i = index; i <= maxSize; i++)
  {
    *(mainFunctArray + index) = *(mainFunctArray+ (index + 1));
...
bool removeFromEnd(string *mainFunctArray, int elementsInArray)
{
  *(mainFunctArray + elementsInArray) = " ";
  return 1;
}


bool insertAt(string *mainFunctArray, string mainFunctChoice, int index, int elementsInArray)
{
  int maxSize = elementsInArray;

  for(int i = index; i <= maxSize; i++)
  {
    *(mainFunctArray +(elementsInArray + 1)) = *(mainFunctArray + elementsInArray);

    elementsInArray--;
  }

  *(mainFunctArray + index) = mainFunctChoice;

  return 1;
}


bool insertAtEnd(string *mainFunctArray, string mainFunctChoice, int elementsInArray)
{
  *(mainFunctArray +(elementsInArray +1)) = mainFunctChoice;

  return 1;
}


bool insertAtZero(string *mainFunctArray, string mainFunctChoice, int elementsInArray)
{
  int maxSize = elementsInArray;

  for(int index = 0; index <= maxSize ; index ++)
  {
  *(mainFunctArray + 1) = *(mainFunctArray + elementsInArray);
  elementsInArray--;
  }
  *(mainFunctArray) = mainFunctChoice;

  return 1;
}


string *expand(string *ptr,int &size)
{
  string *temp = nullptr;
  ...
  for(int location = 0; location < numElems; location++)
  {*(temp + location) = *(ptr + location);}
